...
def build_student():
    ...
    for slice_num in range(5):
        ...
        job_name = f'{job_name}_SSVD_TRP_{config["loss"]["coef_student"]}{config["loss"]["contrastive_type"]}{config["loss"]["ssvd_trip_alpha"]}_MC_{config["sampling"]["mc_dropout_iters"]}_CL_{config["loss"]["loss_type"]}{config["loss"]["balanced_beta"]}'
