...
def train_PUL(ckpt_path, excel_path, teacher_path, config):
    ...
    while epoch < train_config['self_train_max_epoch'] + 1:
        ...
        if os.path.exists(best_model_path):
            ...
            load_checkpoint(model, teacher_path, logger)
        selected_ids_, pseudo_labels_, conf_scores, y_var, unselected_ids, unselected_labels, selected_prob, unselected_prob = unlabeled_data_sampler.select_samples(model)
        selected_ids = selected_ids_[pseudo_labels_==0]
        pseudo_labels = pseudo_labels_[pseudo_labels_==0]
        ...
        selected_confusion = confusion_matrix(true_label_selected, pseudo_labels_)
        ...
        kappa = cohen_kappa_score(true_label_selected, pseudo_labels_)
        mcc = matthews_corrcoef(np.array([1 if x == 1 else -1 for x in true_label_selected]), np.array([1 if x == 1 else -1 for x in pseudo_labels_]))
...
def train(unlabel_train_loader, label_train_loader, model, optimizer, scheduler, metric_meter, config, loss_fun,
    ...
    for i, batch in enumerate(unlabel_train_loader, start=1):
        ...
        try:
            ...
            print('grad contains nan, skip to next batch')
