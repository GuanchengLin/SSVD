...
def train_PUL(ckpt_path, excel_path, teacher_path, config):
    ...
    positive_label_num = {'big-vul': 774, 'Devign': 894, 'reveal': 190, 'Juliet': 1199}
    dataset_name = ''
    if 'big-vul' in data_config["ssl_data_path"]:
        dataset_name = 'big-vul'
    elif 'Devign' in data_config["ssl_data_path"]:
        dataset_name = 'Devign'
    elif 'reveal' in data_config["ssl_data_path"]:
        dataset_name = 'reveal'
    else:
        dataset_name = 'Juliet'

    ### Get data loader
    # train_dset = GraphDataset(f'{data_config["ssl_data_path"]}/{data_config["label_rate"]}/train.pkl', mode='train')
    ...
    train_loader = DataLoader(train_dset, batch_size=train_config['batch_size'], shuffle=True, pin_memory=False,
                              num_workers=4)
    ...
    logger.print(
        'Length of val/unlabeled/test datasets: {}, {}, {}'.format(len(val_dset), len(unlabeled_dset), len(test_dset)))
    logger.print('Length of val/unlabeled/test dataloaders: {}, {}, {}'.format(len(val_loader),
                                                                               int(1 / sample_config['sampling_rate']),
                                                                               len(test_loader)))
    ...
    logger.print(
        ...
        'recall: {:5f}%, f1: {:5f}%, mcc: {:5f}%, kappa: {:5f}%, auc: {:5f}%, TN: {:d}, FP: {:d}, FN: {:d}, TP: {:d}'.format(
            ...
            teacher_scores['kappa'], teacher_scores['auc'], confusion[0][0], confusion[0][1], confusion[1][0],
            confusion[1][1]))
    ...
    while epoch < train_config['self_train_max_epoch'] + 1:
        ...
        if os.path.exists(best_model_path):
            ...
            load_checkpoint(model, teacher_path, logger)
        selected_ids, pseudo_labels, conf_scores, y_var, unselected_ids, unselected_labels, selected_prob, unselected_prob = unlabeled_data_sampler.select_samples(
            model)
        selected_ids = selected_ids[pseudo_labels==0]
        pseudo_labels = pseudo_labels[pseudo_labels==0]
        ...
        samples_per_class = [sum((pseudo_labels == 0)), positive_label_num[dataset_name]]
        ...
        tn, fp, fn, tp = selected_confusion[0][0], selected_confusion[0][1], selected_confusion[1][0], \
                         selected_confusion[1][1]
        ...
        tn, fp, fn, tp = unselected_confusion[0][0], unselected_confusion[0][1], unselected_confusion[1][0], \
                         unselected_confusion[1][1]
        accuracy = (tn + tp) / (tn + fp + fn + tp)
        precision = tp / (tp + fp) if (tp + fp) != 0 else 0
        recall = tp / (tp + fn) if (tp + fn) != 0 else 0
        f1 = (2 * precision * recall) / (precision + recall) if (precision + recall) != 0 else 0
        ...
        while True:
            ...
            train(unlabeled_data_loader, train_loader, model, optimizer, scheduler, metric_meter, loss_config, loss_fun,
                  samples_per_class)
            ...
            if save_ckpt:
                ...
                if train_config['stopper_mode'] == 'f1acc':
                    test_acc_dif = scores['accuracy'] - teacher_test_acc
                    test_f1_dif = scores['f1'] - teacher_test_f1
                    best_model_path = os.path.join(ckpt_path,
                                                   'best_student_{:.5f}_{:.5f}.pth'.format(test_acc_dif, test_f1_dif))
                    save_checkpoint(model, path=best_model_path, logger=logger)
                    logger.print(
                        f'acc diff = {test_acc_dif:.5f} f1 diff = {test_f1_dif:.5f} achieved. Checkpoint saved.')
                if train_config['stopper_mode'] == 'recallacc':
                    test_acc_dif = scores['accuracy'] - teacher_test_acc
                    test_recall_dif = scores['recall'] - teacher_test_recall
                    best_model_path = os.path.join(ckpt_path, 'best_student_{:.5f}_{:.5f}.pth'.format(test_acc_dif,
                                                                                                      test_recall_dif))
                    save_checkpoint(model, path=best_model_path, logger=logger)
                    logger.print(
                        f'acc diff = {test_acc_dif:.5f} recall diff = {test_recall_dif:.5f} achieved. Checkpoint saved.')
            if epoch_early_stopper.early_stop:
                break
        self_training_early_stopper(epoch_early_stopper.max_mcc, epoch_early_stopper.min_loss,
                                    epoch_early_stopper.max_f1, epoch_early_stopper.max_acc,
                                    epoch_early_stopper.max_recall)
...
def train(unlabel_train_loader, label_train_loader, model, optimizer, scheduler, metric_meter, config, loss_fun,
          samples_per_class):
    device = model.device
    model.train()
    label_epoch = len(label_train_loader)
    # optimizer.zero_grad()
    ...
    for i, batch in enumerate(unlabel_train_loader, start=1):
        ...
        label_token_type = labeled_batch['token_type_ids'].to(device)
        label_attention_mask = labeled_batch['attention_mask'].to(device)
        ...
        label_output = model(label_input_ids, label_token_type, label_attention_mask)
        label_logits = label_output['logits']
        if i <= label_epoch:
            logits = torch.concatenate([logits, label_logits])
            label = torch.concatenate([label, label_label])
        ...
        metric_meter.update('class_loss', loss.item(), total=1)
