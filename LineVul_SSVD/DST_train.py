...
from .dst_model import BaseModel
...
def train_DST(ckpt_path, excel_path, teacher_path, config):
...
def train(unlabel_train_loader, label_train_loader, model, optimizer, scheduler, metric_meter, config, loss_fun, samples_per_class):
    ...
    model.train()
    optimizer.zero_grad()
    worst_case_criterion = WorstCaseEstimationLoss(eta_prime=2)
    for i, batch in enumerate(unlabel_train_loader, start=1):
        ...
        batch_l = next(label_train_loader)
        input_ids_l = batch_l['input_ids'].to(device)
        token_type_ids_l = batch_l['token_type_ids'].to(device)
        attention_mask_l = batch_l['attention_mask'].to(device)
        label_l = batch_l['label'].to(device)
        selected_ids = []

        output = model(input_ids, token_type_ids, attention_mask)
        output_l = model(input_ids_l, token_type_ids_l, attention_mask_l)

        # logits = output['logits']
        pseudo_out, worst_out_u, main_out_u = output['pseudo_out'], output['worst_out'], output['logits']
        main_out_l, worst_out_l = output_l['logits'], output_l['worst_out']

        labels_np = label.cpu().detach().numpy()
        output_np = pseudo_out.cpu().argmax(1).detach().numpy()
        weight = (-torch.log(batch['variance'] + 1e-10) * 0.1).to(device)

        if config['loss_type'] == 'FocalLoss':
            class_loss = loss_fun(pseudo_out, label, weight)
            ...
            class_loss = focal_loss(pseudo_out, label)
        elif config['loss_type'] == 'NRCE':
            class_loss = NRCE(pseudo_out, label, beta=config['balanced_beta'], samples_per_class=samples_per_class, k=config['BPH_k'], reduction='none')
        else:
            class_loss = F.cross_entropy(pseudo_out, label, reduction='none')
        ...
        worst_case_loss = 0.3 * worst_case_criterion(main_out_l, worst_out_l, main_out_u,
                                                                           worst_out_u)
        loss = class_loss + config['coef_student'] * contrastive_loss + worst_case_loss
        ...
        metric_meter.update('accuracy',
                            correct=(main_out_u.argmax(1) == label).sum().item(), total=len(label))
