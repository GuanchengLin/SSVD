...
def train_teacher(ckpt_path, config):
    ...
    train_loader = DataLoader(train_dset, batch_size=train_config['batch_size'], shuffle=True, pin_memory=True,
                              num_workers=4)
    ...
    val_loader = DataLoader(val_dset, batch_size=train_config['eval_batch_size'], pin_memory=True, num_workers=4)
    test_loader = DataLoader(test_dset, batch_size=train_config['eval_batch_size'], pin_memory=True, num_workers=4)

    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    model = BaseModel(train_config['transformers_model_name'], num_classes=num_classes, output_hidden_states=True,
    ...
    while True:
        ...
        if train_steps >= 5:
            ...
            if save_ckpt:
                save_checkpoint(model, path=os.path.join(ckpt_path, '_best_teacher.pth'))
    ...
    model_path = os.path.join(ckpt_path, '_best_teacher.pth')
    ...
    os.rename(
        os.path.join(ckpt_path, '_best_teacher.pth'),
        os.path.join(ckpt_path, f'best_teacher.pth')
    )
    logger.print('\n' * 5)
